    ...
    if supervise:
        if (
            ...
            if job == "server_calls":
                ...
                logger.warning(
                    "You're about to leave the session. " "InstaPy2 will exit soon!"
                )
                exit()
    ...
    if action == "sleep":
        if interval == "hourly":
            ...
            message = (
                ...
                "\ttime for InstaPy2 to take a big good nap :-)".format(
                    job, "%.1f" % (nap / 60 / 60)
                )
            )
...
def get_icons():
    ...
    icons_path = get_pkg_resource_path("instapy2", "icons/")
